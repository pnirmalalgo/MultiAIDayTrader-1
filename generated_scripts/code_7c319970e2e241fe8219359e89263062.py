...
for ticker in data['Ticker'].unique():
    ...
    for i in range(len(ticker_data)):
        if ticker_data['RSI'].iloc[i] < 25 and not in_position:
            buy_signals[i] = ticker_data['Close'].iloc[i]
            in_position = True
        elif ticker_data['RSI'].iloc[i] > 75 and in_position:
    ...
    shares = 0
    portfolio_value = pd.Series(index=ticker_data.index, data=np.nan)

    for d in ticker_data.index:
        if d in buy_prices.index:
            shares = int(cash_balance / ticker_data['Close'].loc[d])
            cash_balance -= shares * ticker_data['Close'].loc[d]
        if d in sell_prices.index:
            cash_balance += shares * ticker_data['Close'].loc[d]
            shares = 0
        portfolio_value.loc[d] = cash_balance + shares * ticker_data['Close'].loc[d]
    ...
    fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['Buy'], mode='markers', marker=dict(symbol='triangle-up', color='green', size=10), name='Buy Signal'), row=1, col=1)
    fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['Sell'], mode='markers', marker=dict(symbol='triangle-down', color='red', size=10), name='Sell Signal'), row=1, col=1)
...
pd.DataFrame(results).to_html('trading_results.html', index=False)