...
for ticker in tickers:
    ...
    ticker_data['Signal'] = ta.trend.MACD(ticker_data['Close']).macd_signal()
    ...
    for i in range(1, len(ticker_data)):
        if not in_position and ticker_data['MACD'].iloc[i-1] < ticker_data['Signal'].iloc[i-1] and ticker_data['MACD'].iloc[i] > ticker_data['Signal'].iloc[i]:
            ticker_data['Buy'].iloc[i] = ticker_data['Close'].iloc[i]
            in_position = True
        elif in_position and ticker_data['MACD'].iloc[i-1] > ticker_data['Signal'].iloc[i-1] and ticker_data['MACD'].iloc[i] < ticker_data['Signal'].iloc[i]:
    ...
    volatility = daily_rets.std() * (252 ** 0.5) * 100 if not daily_rets.empty else 0
    max_drawdown = (1 - cumulative_curve / cumulative_curve.cummax()).max() * 100 if not cumulative_curve.empty else 0
    ...
    fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['MACD'], name='MACD', line=dict(color='orange')), row=1, col=1, secondary_y=True)
    fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['Signal'], name='Signal', line=dict(color='red')), row=1, col=1, secondary_y=True)
