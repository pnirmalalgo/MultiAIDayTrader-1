...
for ticker in tickers:
    ...
    for i in range(len(ticker_data)):
        if not in_position and ticker_data['MA10'].iloc[i] > ticker_data['MA30'].iloc[i]:
            ticker_data['Buy'].iloc[i] = ticker_data['Close'].iloc[i]
            in_position = True
        elif in_position and ticker_data['MA10'].iloc[i] < ticker_data['MA30'].iloc[i]:
    ...
    cumulative_curve = pd.Series((1 + returns).cumprod(), index=sell_prices.index)
    ...
    for date in ticker_data.index:
        if date in buy_prices.index:
            shares = int(cash_balance / buy_prices[date])
            cash_balance -= shares * buy_prices[date]
        if date in sell_prices.index:
            cash_balance += shares * sell_prices[date]
            shares = 0
        portfolio_value[date] = cash_balance + shares * ticker_data['Close'].loc[date]

    cumulative_return = (portfolio_value.iloc[-1] - initial_capital) / initial_capital * 100
    total_days = (portfolio_value.index[-1] - portfolio_value.index[0]).days
    annualized_return = ((portfolio_value.iloc[-1] / initial_capital) ** (365 / total_days) - 1) * 100 if total_days > 0 else 0
    ...
    fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['Buy'], mode='markers', name='Buy', marker=dict(color='green', symbol='triangle-up')), row=1, col=1)
    fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['Sell'], mode='markers', name='Sell', marker=dict(color='red', symbol='triangle-down')), row=1, col=1)
    ...
    fig.update_layout(title=f'Trading Strategy for {ticker}', xaxis_title='Date', yaxis_title='Price')
