...
for ticker in tickers:
    ...
    if len(buy_prices) > 0:
        ...
        shares = 0
        portfolio_value = pd.Series(index=ticker_data.index, data=0)
        ...
        fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['Close'], mode='lines', name='Close Price'), secondary_y=False)
        fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['RSI'], mode='lines', name='RSI'), secondary_y=True)
        fig.add_trace(go.Scatter(x=buy_prices.index, y=buy_prices.values, mode='markers', marker=dict(symbol='triangle-up', color='green', size=10), name='Buy Signal'), secondary_y=False)
        fig.add_trace(go.Scatter(x=sell_prices.index, y=sell_prices.values, mode='markers', marker=dict(symbol='triangle-down', color='red', size=10), name='Sell Signal'), secondary_y=False)
        fig.add_trace(go.Scatter(x=portfolio_value.index, y=portfolio_value, mode='lines', name='Portfolio Value'), secondary_y=False)
