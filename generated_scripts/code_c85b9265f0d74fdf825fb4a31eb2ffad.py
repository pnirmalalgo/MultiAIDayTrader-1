...
for ticker in tickers:
    ...
    ticker_data['MA20'] = ticker_data['Close'].rolling(window=20).mean().shift(1)
    ticker_data['MA50'] = ticker_data['Close'].rolling(window=50).mean().shift(1)
    ticker_data['Buy'] = None
    ticker_data['Sell'] = None
    in_position = False

    for i in range(len(ticker_data)):
        if not in_position and (ticker_data['MA20'].iloc[i] > ticker_data['MA50'].iloc[i]):
            ticker_data['Buy'].iloc[i] = ticker_data['Close'].iloc[i]
            in_position = True
        elif in_position and (ticker_data['MA50'].iloc[i] > ticker_data['MA20'].iloc[i]):
    ...
    fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['MA20'], mode='lines', name='20-day MA', line=dict(dash='dash')), row=1, col=1)
    fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data['MA50'], mode='lines', name='50-day MA', line=dict(dash='dash')), row=1, col=1)
